...
type L7PlotCtor<O extends PlotOptions, P extends Plot<O>> = new (
...
>(Ctor: L7PlotCtor<O, P>, config: C) {
  ...
  useEffect(() => {
    ...
    changeData =
      !isEqual(currentSourceConfig, inputSourceDataConfig) || currentSourceData !== inputSourceData;
    updateOption = !isEqual(currentConfig, inputConfig);
    plotConfig.current = deepCloneMapConfig<C>(config);
    if (updateMap) {
      const updateMapConfig = pick(inputMap as any, [
        'center',
        'pitch',
        'rotation',
        'zoom',
    ...
    if (updateOption) {
      if (plotRef.current.loaded) {
        plotRef.current.update(inputConfig as any);
      } else {
        plotRef.current.once('loaded', () => {
          plotRef.current?.update(inputConfig as any);
        });
  ...
  useEffect(() => {
    ...
    if (onReady) {
      plotRef.current.once('loaded', () => {
        onReady(plotRef.current as any);
      });
    }
