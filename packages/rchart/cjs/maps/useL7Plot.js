...
function useL7Plot(Ctor, config) {
  ...
  (0, _react.useEffect)(() => {
    if (!plotRef.current || !plotConfig.current || (0, _util.isEqual)(plotConfig.current, config))
      return;
    ...
    changeData =
      !(0, _util.isEqual)(currentSourceConfig, inputSourceDataConfig) ||
      currentSourceData !== inputSourceData;
    updateOption = !(0, _util.isEqual)(currentConfig, inputConfig);
    plotConfig.current = (0, _util2.deepCloneMapConfig)(config);

    if (updateMap) {
      const updateMapConfig = (0, _util.pick)(inputMap, [
        'center',
        'pitch',
        'rotation',
        'zoom',
      ...
      if (plotRef.current.loaded) {
        ...
        plotRef.current.once('loaded', () => {
          ...
          (_plotRef$current = plotRef.current) === null || _plotRef$current === void 0
            ? void 0
            : _plotRef$current.changeData(inputSourceData, inputSourceDataConfig);
        });
      }
    }
    ...
    if (updateOption) {
      if (plotRef.current.loaded) {
        ...
        plotRef.current.once('loaded', () => {
          ...
          (_plotRef$current2 = plotRef.current) === null || _plotRef$current2 === void 0
            ? void 0
            : _plotRef$current2.update(inputConfig);
        });
      }
    }
  }, [config]);
